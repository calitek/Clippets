...
function _setCurrentSnips(treeNode, _snipData) {
  let snipsRecord = lodash.find(_snipData.allSnips, {nodeid: treeNode.nodeid});
...
function _newTreeNode(newNodeID, _snipData) {
  let newSnips = {nodeid: newNodeID.nodeID, snips: []};
...
export default function handleActions(state = initialSnipState, action) {
  ...
  switch (action.type) {
    ...
    case 'SelectTreeNode': return _setCurrentSnips(action.node, _snipData);
    case 'SelectSnipItem': return _selectSnipItem(action.item, _snipData);
    case 'SaveSnipEdit': return _saveSnipEdit(action.item, _snipData);
