...
export function treeActions(action) {
  switch (action) {
    ...
    default: return treeChangeActions(action);
  }
}
...
export function treeChangeActions(action) {
  return (dispatch, getState) => {
    switch (action) {
      case 'moveUp': dispatch({type: 'MoveNodeUp'}); break;
      case 'moveDown': dispatch({type: 'MoveNodeDown'}); break;
      case 'remove': {
        let oldNodeID = getState().treeData.currentTreeNode.nodeid;
